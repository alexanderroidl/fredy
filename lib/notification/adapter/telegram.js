...
export const send = ({
  ...
  const { token, chatId } = notificationConfig.find(
  ...
  // ensures one request per second
  const throttle = pThrottle({ limit: 1, interval: RATE_LIMIT_INTERVAL });
  const sendMessage = throttle(async (message) => {
