...
const providersDir = './lib/provider';
const providers = fs.readdirSync(providersDir).filter((fileName) => fileName.endsWith('.js'));
...
const fetchedProviders = await Promise.all(
  providers.filter((provider) => provider.endsWith('.js')).map(async (pro) => import(`${providersDir}/${pro}`)),
);
    ...
    if (!config.demoMode) {
      if (isDuringWorkingHoursOrNotSet) {
        ...
        jobStorage
          ...
          .forEach((job) => {
            job.provider
              .filter((p) => fetchedProviders.find((fp) => fp.metaInformation.id === p.id) != null)
              .forEach(async (prov) => {
                const pro = fetchedProviders.find((fp) => fp.metaInformation.id === prov.id);
